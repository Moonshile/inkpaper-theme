...
All colors, fonts, and spacing are defined as CSS custom properties in the `@inkpaper/core` package. This package is pure CSS, framework-agnostic. `@inkpaper/vitepress` and `@inkpaper/astro` each build on top of it with their respective framework adapters.

The benefit of this split: the color system and texture are fully shared between the two framework versions. Only the component layer is implemented separately.
