...
Markdown 的 `![](url)` 图片语法可以直接嵌入视频。当 URL 以视频扩展名（`.mp4`、`.webm`、`.mov`、`.avi`、`.mkv`）结尾时，Astro 集成内置的 remark 插件会自动将图片节点转换为 `<video>` 标签（带 `controls` 属性）。

```md
![](demo.mp4)
```

无需任何配置——`remarkVideo` 插件已由 `@inkpaper/astro` 集成自动应用。如需单独使用：
