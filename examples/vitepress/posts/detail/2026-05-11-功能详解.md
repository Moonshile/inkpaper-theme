...
Markdown 的 `![](url)` 图片语法可以直接嵌入视频。当 URL 以视频扩展名（`.mp4`、`.webm`、`.mov`、`.avi`、`.mkv`）结尾时，渲染器会自动输出 `<video>` 标签（带 `controls` 属性），而不是 `<img>` 标签。

```md
![](demo.mp4)
```

在 VitePress 配置中启用视频插件：
...
`alt` 文本或 `title` 属性会自动成为 `<video>` 的 `title` 属性：

```md
![墨迹效果的演示](demo.mp4)
```

视频和图片共享 `@inkpaper/core` 中定义的响应式样式（max-width、border-radius）。
